...
# amount = 0
# subtotal_entry = 0
# sgst_entry,grand_total_entry=0,0
# customer_name_entry,date_entry,mo_no_entry = 0,0,0
def billbook():
    def topframe(root):
    ...
    def middleframe(root):
        global customer_name_entry,date_entry,mo_no_entry
        ...
        mo_no_label = Label(middle_frame,text="Mob No.",font="arial 12 bold",bg="white")
        mo_no_label.place(x=890,y=25)

        mo_no_entry=Entry(middle_frame,text="",width=20,relief=SOLID)
        mo_no_entry.place(x=970,y=25)
        ...
        date_entry=Entry(middle_frame,text="",width=20,relief=SOLID)
    ...
    def thirdframe(root):
        global width_list,x_list,no,descritption,qty,rate,gstrate,amount,amount_entry
    ...
    def lastframe(root):
        global subtotal_entry,sgst_entry,grand_total_entry
    ...
    def getvalue():
        ...
        len1 = findleghnth()
        for i in range(len1):
            # print(no1[i][2].get(),no1[i][3].get())
            am = int(no1[i][2].get()) * int(no1[i][3].get())
            if no1[i][6]:
                no1[i][6].insert(0,am)
            else:
                no1[i][6].insert(0,0)
        ...
        subtotal_entry.insert(0,subt)
        gst = subt*0.05
        sgst_entry.insert(0,gst)
        g_total =subt+gst
        grand_total_entry.insert(0,(g_total))
    ...
    def main():
        ...
        thirdframe(root)
        lastframe(root)
        t1 =Button(root,text="Total",command = getvalue,width = 10)
        t1.place(x=900,y=560)
        root.mainloop()
